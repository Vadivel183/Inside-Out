...
function ExpenseRow({ row, onChange, onRemove, canRemove }) {
  return (
    <div className="et-row">
      ...
      <input
        className="et-input et-note"
        placeholder="Note (optional)"
        value={row.note}
        onChange={e => onChange(row.id, "note", e.target.value)}
      />
      <button
        className={`et-row-remove${canRemove ? "" : " disabled"}`}
        onClick={() => canRemove && onRemove(row.id)}
        title="Remove row"
      >✕</button>
    </div>
  )
...
export default function ExpenseTracker() {
  ...
  const [dailyLog, setDailyLog]       = useState({ [todayStr()]: [emptyRow()] })
  ...
  const [monthlyLog, setMonthlyLog] = useState({ [monthKey]: [emptyRow()] })
  ...
  const submitDeferred = () => {
    ...
    if (!name || !amount || !category) {
      setDeferAlert("Please fill Name, Amount and Category."); return
    }
    ...
    months.forEach(({ m: mo, y: yr }) => {
      ...
      setMonthlyLog(prev => {
        const cleaned = (prev[k] || [emptyRow()]).filter(r => r.name || r.amount)
        const updated = [...cleaned, row]
        // TODO: POST /api/expenses/monthly  { month: mo, year: yr, rows: updated }
        console.log(`[Deferred] ${MONTHS[mo]} ${yr}`, updated)
        return { ...prev, [k]: updated }
      })
    })

    // TODO: POST /api/expenses/deferred  { splits, meta }
    setDeferredSubmitted(prev => [...prev, { ...deferred, months: months.length, split, id: uid() }])
    setDeferred(initDeferred())
    const msg = `✓ ${formatINR(amt)} split across ${months.length} months and logged.`
    setDeferAlert(msg)
    showToast(msg)
  ...
  const dayTotal = dayRows.reduce((s, r)  => s + (parseFloat(r.amount) || 0), 0)
  ...
  return (
    <div className="et-page">
      ...
      <div className="et-section">
        ...
        <div className="et-section-footer">
          ...
          <div className="et-footer-right">
            ...
            <button className="et-submit-btn" onClick={() => {
              // TODO: POST dailyLog[currentDate] to backend
              showToast(`Day log for ${formatDate(currentDate)} saved — ${dayRows.filter(r => r.name || r.amount).length} entries`)
            }}>Save Day Log ↑</button>
          </div>
        </div>
      </div>
      ...
      <div className="et-section">
        ...
        <div className="et-section-footer">
          ...
          <div className="et-footer-right">
            ...
            <button className="et-submit-btn" onClick={() => {
              // TODO: POST monthlyLog[monthKey] to backend
              showToast(`${MONTHS[monthIdx]} ${monthYear} log saved — ${mRows.filter(r => r.name || r.amount).length} entries`)
            }}>Save Month Log ↑</button>
          </div>
        </div>
      </div>
      ...
      <div className="et-section">
        ...
        <div className="et-defer-form">
          ...
          <div className="et-defer-actions">
            ...
            <button className="et-submit-btn" onClick={submitDeferred}>
              Split &amp; Log Expense →
            </button>
          </div>
        </div>

        {/* Logged deferred entries */}
      ...
      {/* Toast — portalled to document.body so fixed positioning works */}
      {toast && createPortal(
